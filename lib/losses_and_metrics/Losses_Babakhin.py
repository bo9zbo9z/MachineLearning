...
Forked and modified for TensorFlow 2.x (very minor changes)
...
def lovasz_loss(y_true, y_pred):
    y_true, y_pred = K.cast(K.squeeze(y_true, -1), 'int32'), \
...
def lovasz_hinge(logits, labels, per_image=True, ignore=None):
    ...
    Binary Lovasz hinge loss
      logits: [B, H, W] Variable, logits at each pixel (between -infty
              and +infty)
...
def lovasz_hinge_flat(logits, labels):
    """
    Binary Lovasz hinge loss
      logits: [P] Variable, logits at each prediction (between -infty
              and +infty)
